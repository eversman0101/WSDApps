	...
	private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback()
	{
		...
		public void onCharacteristicRead(BluetoothGatt gatt,BluetoothGattCharacteristic characteristic, int status)
		{
			if (status == BluetoothGatt.GATT_SUCCESS)
			{
				...
				Log.d("ble读取的数据", ByteUtil.BinaryToHexString(sucString));
		...
		public void onCharacteristicChanged(BluetoothGatt gatt,BluetoothGattCharacteristic characteristic)
		{
			Log.i("ble响应的数据", ByteUtil.BinaryToHexString(characteristic.getValue()));
			broadcastUpdate(ACTION_DATA_AVAILABLE, characteristic);
		}
        /*
         * 特征值的写
		...
		{
			Log.i(TAG, "发送了数据：" + ByteUtil.BinaryToHexString(characteristic.getValue()));
		...
		if (mBluetoothAdapter == null || mBluetoothGatt == null)
		{
            ...
            if(descriptorList != null && descriptorList.size() > 0) {
                for(BluetoothGattDescriptor descriptor : descriptorList) {
                    ...
                    Log.d("BluetoothLeService","已设置Descriptor");
                }
            }
            boolean status=mBluetoothGatt.setCharacteristicNotification(characteristic, true);
            Log.d("BluetoothLeService", "Notification状态:"+status);
